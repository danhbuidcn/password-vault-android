...
- Chỉnh thêm theo góp ý trực tiếp (2026-07-26): Tag ở dòng List chuyển từ hàng riêng bên dưới lên đúng vị trí icon cảnh báo cũ (hàng đầu, bên phải tên/username) — `VaultItemCard` trong `VaultScreen.kt`.
- README.md `Screenshots/` chụp lại toàn bộ theo giao diện mới nhất (Setup/Unlock/List/Detail/Form/Settings); mục "Tính năng chính" bỏ nhắc custom field/template.
